...
function RecentAddItem({ product }: { product: Product }) {
  ...
  return (
    <div className="group flex cursor-pointer items-center gap-3 rounded-lg p-3 transition-colors hover:bg-muted/50 dark:hover:bg-neutral-800/50">
...
function ProductRow({ product }: { product: Product }) {
  ...
  return (
    <TableRow className="transition-colors hover:bg-muted/50">
      ...
      <td className="p-4 text-sm font-medium text-foreground">{formatCurrency(product.price)}</td>
      <td className="p-4">
        <Badge className={`gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${currentStatus.color}`}>
          <span className={`size-1.5 rounded-full ${currentStatus.dot}`}></span>
          {currentStatus.label}
        </Badge>
      </td>
    ...
    <>
      <div className="flex flex-wrap items-end justify-between gap-6">
        ...
        <div className="flex gap-3">
          <Button variant="outline" className="gap-2 rounded-full border-border px-5 py-3 font-medium">
        ...
        <div className="flex flex-col gap-6 xl:col-span-4">
          ...
          <div className="flex flex-1 flex-col rounded-lg border bg-card shadow-sm dark:bg-[#1a190b]">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h2 className="text-sm font-bold uppercase tracking-wider text-foreground">Recentes</h2>
              <a href="#" className="text-xs font-bold text-primary hover:underline">Ver Todos</a>
            </div>
